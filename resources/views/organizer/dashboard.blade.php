...
@section('page_subtitle', 'Ringkasan performa event, transaksi, dan tiket untuk organisasi Anda.')
    ...
    <div class="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
        <div class="p-8 md:flex md:items-center md:justify-between gap-6">
            <div>
                ...
                <p class="mt-2 text-slate-500 max-w-2xl">Pantau pendapatan, transaksi, dan event aktif dari satu tempat.</p>
            </div>
        ...
        <div class="p-8 border-b">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                ...
                <div class="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-emerald-50 text-emerald-700 font-semibold text-sm">
                    ...
                    Tingkat keberhasilan {{ $successRate ?? 0 }}%
                </div>
            </div>
        </div>
        <div class="p-8">
            @if(isset($monthlyRevenue) && count($monthlyRevenue) > 0)
                <div class="flex items-end gap-4 h-64">
                    @foreach($monthlyRevenue as $item)
                        @php $height = ($maxMonthlyRevenue ?? 0) > 0 ? max(10, round(($item['value'] / $maxMonthlyRevenue) * 100)) : 10; @endphp
    ...
    <div class="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
        ...
        <div class="p-8 space-y-5">
            ...
            <div class="rounded-2xl bg-emerald-50 p-5">
                ...
                <p class="mt-2 text-3xl font-black text-slate-900">Rp {{ number_format($averageMonthlyRevenue ?? 0, 0, ',', '.') }}</p>
            </div>
            <div class="rounded-2xl bg-amber-50 p-5">
                ...
                <p class="mt-2 text-3xl font-black text-slate-900">{{ $totalSuccessfulTransactions ?? 0 }} / {{ $totalTransactions ?? 0 }}</p>
            </div>
        </div>
    </div>
</div>
