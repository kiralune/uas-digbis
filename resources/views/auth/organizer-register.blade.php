...
<main class="min-h-screen flex items-center justify-center px-4 py-10 bg-slate-50">
    <div class="max-w-3xl w-full rounded-[2rem] bg-white p-10 shadow-2xl border border-slate-200">
        <div class="mb-10 text-center">
            <h1 class="text-4xl font-black text-slate-900">Daftar Sebagai Organizer</h1>
        ...
        <form action="{{ route('organizer_auth.register.post') }}" method="POST" class="space-y-6">
            ...
            <button type="submit"
                    class="w-full rounded-2xl bg-indigo-600 py-4 text-white font-bold shadow-lg shadow-indigo-200 hover:bg-indigo-700 transition">Daftar</button>
        </form>

        <div class="mt-8 text-center text-slate-500">
            Sudah punya akun Organizer? <a href="{{ route('organizer_auth.login') }}" class="text-indigo-600 font-semibold">Masuk Organizer</a>
        </div>
    </div>
</main>
@endsection
