...
<main class="max-w-3xl mx-auto px-6 py-20 text-center">
    <div class="bg-white rounded-3xl border border-slate-200 p-12 shadow-sm inline-block w-full max-w-md">
        ...
        <div class="flex flex-col gap-3">
            <a href="{{ route('ticket', ['email' => $transaction->customer_email]) }}" class="inline-block px-8 py-4 bg-indigo-600 text-white rounded-xl font-bold hover:bg-indigo-700 transition">
                Lihat Tiket Saya
            </a>
            <a href="{{ route('home') }}" class="inline-block px-8 py-4 bg-slate-100 text-slate-700 rounded-xl font-bold hover:bg-slate-200 transition">
                Kembali ke Beranda
            </a>
        </div>
    </div>
